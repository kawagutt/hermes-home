...
> **v4 tasks** (`run-dp task start` created `jobs.yaml`): skip this entire section. Session/model evidence is mandatory via **`jobs.yaml`** for all presets; run `run-dp render model-usage` for the display table.

**Pre-v4 only — always fill this small block** (copied task plans break relative links—use plain paths below).
...
**Default policy (pre-v4):** Preset **`standard`** / **`deep`** → **yes** and `artifacts.model_usage` rows via `dp_stage_boundary.py`. Preset **`light`** may set **no** with Reason `no need`. Skipping on standard/deep requires documented human approval in **Reason**.

When **yes**, each primary segment boundary needs a `model_usage` row (`dp_stage_boundary.py --print-markdown-row`). Review workers use **`review_manifest.yaml` only**.
