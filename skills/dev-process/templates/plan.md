...
**Default policy (`dev-process`):** Tasks on preset **`standard`** or **`deep`** MUST set **`Model usage record required?`** to **yes**, bind **`artifacts.model_usage`**, materialize from `skills/dev-process/templates/model_usage.md`, and **append rows** at major stage boundaries (see `skills/dev-process/validation/SKILL.md` § Primary segment boundary completion). Preset **`light`** or **trivial docs-only / mechanical tasks** MAY set **`no`** with **Reason** `no need` and leave **`artifacts.model_usage`** empty. A **`standard`** or **`deep`** plan that sets **`no`** MUST document **explicit human approval** for skipping **`artifacts.model_usage`** (copied rationale in **Reason** plus a pointer such as **`artifacts.timeline`**)—silent waivers are **non-compliant**.

When **yes**, each **primary** segment boundary is not complete until its `model_usage` row exists (`dp_stage_boundary.py --print-markdown-row`). Review rounds (`*_review`, checkpoint reviews) record worker sessions in **`review_manifest.yaml` only**—not in **`artifacts.model_usage`**.
