...
def main() -> int:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show intended state/timeline updates"
    )
    parser.add_argument(
        "--apply", action="store_true", help="Apply state/timeline updates"
    )
    parser.add_argument(
        "--evidence",
        default="",
        help="Command/evidence used to create or switch branch",
    )
    parser.add_argument(
        "--repo",
        default=".",
        help="Repository path used to verify the current git branch",
    )
    parser.add_argument(
        "--skip-git-check",
        action="store_true",
        help="Skip current git branch verification, for isolated tests only",
    )
    ...
    if not timeline_rel:
        raise SystemExit(
            "ERROR artifacts.timeline must be materialized before branch helper apply"
        )
    ...
    if not args.skip_git_check and current != args.branch:
        print(
            f"ERROR current git branch {current!r} does not match expected task branch {args.branch!r}"
        )
        return 1
