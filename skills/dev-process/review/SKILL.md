...
**Humans (or orchestrator recording human choice):** after **any** explicit gate decision (approve or not approved / rework), clear **`pending_human_gate`**. If approved, set the matching canonical gate key: **`approved.human_spec_gate`** or **`approved.final_human_gate`**. If not approved or rework is requested, keep the matching canonical gate key **false**, reset the matching **`reviewed.*`** marker to **false** (`reviewed.spec` for `human_spec_gate`, `reviewed.final` for `final_human_gate`), record in gate artifact / timeline, then set **`current_stage`** to the rework route.
