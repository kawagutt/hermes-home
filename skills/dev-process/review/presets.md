...
## Reasoning effort (Hermes-compatible runtime)

**Bind reasoning effort to the selected review-depth preset.** Authoritative table and guardrails: [validation/SKILL.md — Reasoning effort by review-depth preset](../validation/SKILL.md#reasoning-effort-by-review-depth-preset).

Summary:

```text
light   → default/medium reasoning effort; never high by default; escalate preset if risk grows
standard → default/medium reasoning effort; move to deep (then high) when triggers appear
deep    → high reasoning effort for synthesis, ambiguous blocker triage, architecture/impact, final recommendation
```
...
### Preset value aliases

Some tools or older text may use shorthand. In this skill, canonical preset values in prose are **`light`**, **`standard`**, and **`deep`** only:
